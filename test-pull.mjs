// Standalone local test -- no Netlify round trip, run this immediately
// after capturing fresh ssecurity/cUserId/serviceToken to eliminate any
// deploy/env-var delay as a variable.
//
// Usage:
//   node test-pull.mjs "<ssecurity>" "<cUserId>" "<serviceToken>"
...
if (!ssecurity || !cUserId || !serviceToken) {
  console.error('Usage: node test-pull.mjs "<ssecurity>" "<cUserId>" "<serviceToken>"');
  process.exit(1);
}
...
let params = {
  data: JSON.stringify({ start_time: startTime, end_time: 0, reverse: true }),
};
...
console.log("Requesting:", url.toString().slice(0, 120), "...\n");
...
console.log("Decrypted response:", text.slice(0, 2000));
